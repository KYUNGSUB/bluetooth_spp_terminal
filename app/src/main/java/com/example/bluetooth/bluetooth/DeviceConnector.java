...
import com.example.bluetooth.activity.DeviceControlActivity;
...
//            byte[] ends = new byte[3];	// 끝 수신부 수신 확인
//            ends[0] = 0x0f;
//            ends[1] = 0x0f;
//            String mark = new String(ends, 0, 2);
            String mark = "0F0F";
            while (true) {
                try {
                    // считываю входящие данные из потока и собираю в строку ответа
                    bytes = mmInStream.read(buffer);
                    String readed = Utils.bytesToHex(buffer, bytes);
//                    String readed = new String(buffer, 0, bytes);
                    readMessage.append(readed);

                    // маркер конца команды - вернуть ответ в главный поток
                    if (readed.contains(mark)) {
